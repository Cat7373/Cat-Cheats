...
public class Gui_CreateGive extends GuiScreenBase {
    ...
    protected void actionPerformed(final GuiButton guiButton) {
        if (guiButton.enabled) {
            if (guiButton.id == this.btnExecute.id) {
                CreateGive.giveItem(this.textCommand.getText(), this.numericSlot.getValue());
    ...
    public void drawScreen(final int par1, final int par2, final float par3) {
        ...
        this.drawString(this.fontRendererObj, this.str_slot, 5, 14, 0xFFFFFF);
        this.drawString(this.fontRendererObj, this.str_command, 5, 39, 0xFFFFFF);
    }
